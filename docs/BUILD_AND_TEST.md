...
(서명 없는 바이너리는 `AudioDeviceStart`가 268451843으로 거부된다). Audio 채널은 USB 전용이다.

- SC-A1 맥 Settings → Audio → "Play Mac audio on the tablet" 켜기 → Start Server → 권한 프롬프트 허용
  → 맥에서 소리 재생 → **태블릿에서만** 들리고 맥 스피커는 무음일 것.
- SC-A2 립싱크: 말하는 사람이 있는 영상으로 입 모양과 소리가 맞는지 확인.
- SC-A3 태블릿 Settings → "Mac audio"를 Off → 소리가 즉시 멈추고, 맥은 무음이 아니라 정상 재생으로 복귀.
- SC-A4 Stop Server 또는 시스템 표시기의 Stop Sharing → 화면과 소리가 함께 멈추고 맥 스피커 즉시 복구,
  메뉴가 Start Server로 전환될 것.
- SC-A5 스트리밍 중 헤드폰/블루투스 연결 → 기본 출력 장치가 바뀌면 캡처가 재시작되어 오디오가 복구될 것.
