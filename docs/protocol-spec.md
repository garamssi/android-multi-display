...
`videoScaling`은 태블릿이 그림을 자기 패널에 어떻게 맞출지 알린다.

| 값 | 의미 | 대가 |
|----|------|------|
| `fit` | 그림 전체를 보여준다 | 비율이 다른 쪽에 검은 여백 |
| `fill` | 패널을 꽉 채운다 | 넘치는 가장자리가 잘린다 |

미러는 맥의 기존 화면을 보내므로 두 화면비가 일치할 이유가 없고, **공짜인 선택은 없다** — 모든
픽셀을 지키고 여백을 받거나, 패널을 다 쓰고 가장자리를 잃는다. 늘리는 것은 어느 쪽도 아니므로
선택지가 아니다. 확장 모드는 태블릿 해상도로 디스플레이를 만들기 때문에 두 값이 같은 결과를 낸다.

필드가 없거나 아는 값이 아니면 `fit`으로 간주한다. 모르는 값에 대해 자르면 사용자가 알 수 없는
이유로 화면 일부가 사라진다.

모드는 **맥이 소유한다**. 확장은 맥의 디스플레이 배치를 바꾸고(창이 이동한다), 입력 수용 여부는
서버가 스스로 판단해야 하는 사안이라 클라이언트가 통보할 수 없다.

`videoScaling`도 같은 이유로 맥이 소유한다. 렌더링은 태블릿이 하지만, 선택은 핸드셰이크로만
전달되므로 변경 시 세션이 재시작된다.
