...
class DisplayViewModel @Inject constructor(
    ...
    fun onSurfaceDestroyed() {
        ...
        audioJob?.cancel()
        audioJob = null
        // Closing the socket is what makes the Mac drop its tap; cancelling the job alone
        // leaves the server sending into a connection nobody reads and silences both ends.
    ...
    private fun startStreaming() {
        ...
        // Rendered frames are the audio path's time reference.
    ...
    // Own job: audio is optional, and a Mac that never opens 7103 (routing off, or older
    // than macOS 14.2) must not take the mirror down. The preference is collected, not read
    // once, so turning audio off stops it now rather than at the next reconnect.
    private fun startAudioStreaming() {
        audioJob = viewModelScope.launch {
            settingsRepository.playMacAudio.collectLatest { enabled ->
                ...
                runCatching {
                    audioStream.connect().collect { event -> Log.i(TAG, "audio event: $event") }
